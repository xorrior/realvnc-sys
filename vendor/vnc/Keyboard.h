 ...
 *
 * @return The keysym corresponding to @p unicodeChar, or zero if not
 *     recognized.
 */
VNC_SDK_API vnc_uint31_t vnc_unicodeToKeysym(vnc_uint31_t unicodeChar);


/**
 * Converts a keysym to a unicode - suitable for converting a keysym
 * received on the server in vnc_Server_InputEventsCallback::keyEventCallback.
 *
 * @param keysym The keysym to convert.
 *
 * @return The unicode character corresponding to the keysym, else zero.
 */
VNC_SDK_API vnc_uint31_t vnc_keysymToUnicode(vnc_uint31_t keysym);
